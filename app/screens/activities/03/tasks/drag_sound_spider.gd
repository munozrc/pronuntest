...
var found_sounds := 0:
	set(new_value):
		...
		$TopContainer/SpiderWeb/Display/Statement.text = "Sonidos restantes: %d" % rest


func _ready():
	boxes = $BottomContainer.get_children()
	number_of_boxes = $BottomContainer.get_child_count()
