...
func _ready():
	...
	$ContainerBottom/RecordButton.save_path = $PredictionComponent.recording


func enable_interactions():
	$ContainerBottom/RecordButton.enable = true


func disable_interactions():
	$ContainerBottom/RecordButton.enable = false
...
func _on_request_completed(_result, response_code, _headers, body):
	...
	if score == 0 or phonemes.size() == 0:
		...
		$ContainerBottom/RecordButton.reset()
		$Narrator.play()
		enable_interactions()
		return
	
	if score < 75:
		$Narrator.stream = load("res://assets/voices/04/tu_nariz_no_vibrar.mp3")
	else:
		$Narrator.stream = load("res://assets/voices/statements/bien_hecho.mp3")
	
	$ContainerCenter/Face/ConfettiEffect.start_animation()
	$Narrator.play()
